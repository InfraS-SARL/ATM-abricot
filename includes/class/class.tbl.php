...
Class TTbl
{
	...
	*/
	function draw_index($url, $datatable = "", $idxchamps = "", $post_char = "", $title = "Index", $opt = 'std')
	{
		...
		elseif (($datatable=="")||(isset($Tindexexist['!']))||(isset($Tindexexist['0'])))$r.=$this->cell($this->link('#', $url.'&charIndex=other', 'lien'));
		...
		for ($i=$l;$i<$l+26;$i++) {
			...
			elseif (($datatable=="")||(isset($Tindexexist[chr($i)])))$r.=$this->cell($this->link(chr($i), $url.'&charIndex='.chr($i), 'lien'));
