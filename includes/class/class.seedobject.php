...
{
	const TRIGGER_PREFIX = 'SEEDOBJBECTDOLIBARR_';
	/**
...
class SeedObject extends SeedObjectDolibarr
{
	...
	const TRIGGER_PREFIX = 'SEEDOBJBECT_';
	...
	function init_vars_by_db()
	{
		...
		while ($obj = $this->db->fetch_object($res)) {
			...
			elseif (strpos($obj->Type, 'date')!==false || strpos($obj->Type, 'timestamp')!==false) $type=array('type'=>'date');
	...
	function addFieldsInDb()
	{
		...
		foreach ($TChamps as $champs => $info) {
			if (!in_array($champs, $Tab)) {
				...
				} elseif ($this->isArray($info) || $this->isText($info)) {
